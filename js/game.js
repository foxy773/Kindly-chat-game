...
import {
  getDatabase, ref, set, child, update, remove, get, query, onValue,
...
function generateplatforms() {
  ...
  for (let i = 0; i < numberOfplatforms; i++) {
    ...
    const ob = new Platform(Math.floor(Math.random() * (canvas.width - platformWidth)), platformY, images.platforms); // Random x-axis position between 0 and 600.
    platforms.push(ob);
    platformY -= 100;
  }
...
function generateEnemies() {
  ...
  for (let i = 0; i < numberOfEnemies; i++) {
    const en = new Enemy(Math.floor(Math.random() * (canvas.width - enemyWidth)), enemyY); // Random x-axis position between 0 and 600.
    enemies.push(en);
...
function generateClouds() {
  ...
  for (let i = 0; i < numberOfClouds; i++) {
    const cl = new Cloud(getRandomNumber(canvas.width - 500, canvas.width + 5000), getRandomNumber(0 - canvas.height, canvas.height + 300), cloudHeight, cloudWidth, getRandomNumber(cloudMinSpeed, cloudMaxSpeed), images.clouds[getRandomNumber(0, images.clouds.length - 1)]); // Random x-axis position between 0 and 600.
    clouds.push(cl);
  }
}
...
const max_FPS = 60; // The maximum FPS we want to allow
